...
class UlistingWishlist {
	...
	public static function ulisting_builder_element($data){
		...
		$path = explode('/', ULISTING_WISHLIST_PATH);
		...
		$data['elements'][] = [
			...
			"params"       => [
				"template_path" => "wishlist/add_button",
	...
	public static function get_wishlist_template(){
		$templates = [
			"template_1" => [
				...
				"name" => "Style 1",
				"template" => '<span data-wishlist_id="[id]" onclick="[click]"  class="ulisting-listing-wishlist stm-cursor-pointer [class] [active] "> <i class="fa fa-heart"></i></span> <span class="[class_load] hidden"><i class="fa fa-heart ld ld-heartbeat"></i></span> ',
			]
		];
