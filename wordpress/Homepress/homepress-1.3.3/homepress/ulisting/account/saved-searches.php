...
<div class="container">
    <table class="table ulisting-table">
        <thead>
        <tr>
            <th><?php esc_html_e("Listing type", "homepress")?></th>
            <th><?php esc_html_e("Params", "homepress")?></th>
            <th style="width: 120px;"><?php esc_html_e("Action", "homepress")?></th>
        </tr>
        </thead>
        <tbody>
        <?php foreach ( $searches as $search ):?>
            <tr class="ulisting-search-item-<?php echo esc_attr($search->id)?>">
                ...
                <td class="ulisting-search-item-buttons">
                    <a target="_blank" class="btn btn-info" href="<?php echo esc_url($search->get_url()) ?>"><i class="fa fa-eye" aria-hidden="true"></i></a>
                    <button onclick="delete_search(this, <?php echo esc_attr($search->id)?>)" class="btn btn-danger"><i class="property-icon-trash-alt"></i></button>
                </td>
            </tr>
        <?php endforeach;?>
        </tbody>
...
<?php else : ?>
    <h3 class="text-center p-t-30"><?php _e("You have no saved searches yet!", "homepress")?></h3>
<?php endif; ?>