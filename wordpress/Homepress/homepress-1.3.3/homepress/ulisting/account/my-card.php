...
$hasAccess = !empty($data['publishable_key']) && (isset($stripe_data['secret_key']) && !empty($stripe_data['secret_key']));
...
<?php if ($stripe_data['enabled'] === 'no' || !$hasAccess) :?>
    <div class="account-payment_history_empty">
        <h3><?php esc_html_e("You don't have any Payment methods.", "homepress")?></h3>
    </div>
<?php else: ?>
    <div id="stripe-my-card">
        <div class="container">
            ...
            <div v-if="cards != ''" v-for="card in cards" class="available-cards">
                <div class="stm-row">
                    <div class="stm-col-2">{{card.brand}}</div>
                    <div class="stm-col-2" v-if="card.default"><?php esc_html_e("Default", "homepress"); ?></div>
                    <div class="stm-col-3"><?php esc_html_e("Expired:", "homepress"); ?>
                    ...
                    <div class="stm-col-2">
                        ...
                        <vue-ulist-dropdown v-if="!card.loading" inline-template :visible="card.visible"
                                            @clickout="card.visible = false">
                            <div>
                            <span @click="visible = !visible" class="ulist-dropbtn">
                                ...
                                <div class="ulist-dropdown-content" v-bind:class="{ 'ulist-dropdown-show': visible }">
                                    <span @click="delete_card(card)"
                                          class="item"><?php esc_html_e("Delete", "homepress"); ?></span>
                                    <span v-if="!card.default" @click="make_default(card)"
                                          class="item"><?php esc_html_e("Make default", "homepress"); ?></span>
                                </div>
                            </div>
                        </vue-ulist-dropdown>
                    </div>
            ...
            <div class="panel-custom p-t-30 p-b-30">
                <stripe-card-component ref="stripe_card" inline-template v-bind:publishable_key="publishable_key"
                                       v-on:set-stripe-token-emit="set_stripe_token">
                    <div>
                        <div style="background-color: rgba(0, 0, 0, 0.0392156862745098);padding: 20px 15px;"
                             id="card-element"></div>
                ...
                <div class="account-button-right-position">
                    <button v-if="!loading" @click="add_card"
                            class="homepress-button"><?php esc_html_e("Add Card", "homepress"); ?></button>
                </div>
