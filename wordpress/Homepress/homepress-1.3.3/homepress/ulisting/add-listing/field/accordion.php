...
<div class="ulisting-accordion">
    ...
    <div v-for="(item, index) in attributes.<?php echo esc_attr( $attribute->name ) ?>.data" class="card">
        <div class="card-header">
            <div class="row">
                <div class="col-12 col-sm-10">
                    <input @click="accordion_toggle_open(item, true)" type="text" v-model="item.title" placeholder="<?php esc_html_e( "Title", 'homepress' ); ?>">
                </div>
        ...
        <div v-if="item.is_open" class="card-body">
            ...
            <label><?php _e( "Options", "homepress" ) ?></label>

            <div class="form-group">
                <div v-for=" ( param, _index) in item.options" class="row">
                    <div class="col-3 p-t-5">
                        <input type="text" v-model="param.key" class="form-control" placeholder="<?php _e( "Key", "homepress" ) ?>">
                    </div>
                    <div class="col-3 p-t-5">
                        <input type="text" v-model="param.val" class="form-control" placeholder="<?php _e( "Value", "homepress" ) ?>">
                    </div>
            ...
            <button class="btn btn-success" @click="add_options(item)"><?php _e( "Add option", "homepress" ) ?></button>
            <hr>
            <textarea v-bind:id="'accordion_content_'+item.id" v-model="item.content"></textarea>
        </div>
    </div>
    <div class="p-t-15 text-right">
        <button type="button" class="btn btn-success" @click="add_item_accordion(attributes.<?php echo esc_attr( $attribute->name ) ?>.data)"><?php esc_html_e( ' + Add', 'homepress' ); ?></button>
    </div>
</div>
