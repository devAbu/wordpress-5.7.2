...
if (!empty($auth_code)) {
    ...
    $message = esc_html__('Please make sure you have purchased this theme with the account you registered current token', 'homepress');
}
...
<div class="wrap about-wrap stm-admin-wrap stm-admin-start-screen">
    ...
    <?php homepress_get_admin_tabs(); ?>

    <?php if (empty($auth_code)) { ?>
        <div class="stm-notice">
            <p class="about-description">
                <?php printf(esc_html__('Thank you for choosing %s! Please register it to enable the %1$s demos and theme auto updates. The instructions below must be followed exactly.', 'homepress'), $theme_name); ?>
            </p>
        </div>
    <?php } ?>
    ...
    <form id="stm_item_registration" method="post" action="">
        ...
        <?php submit_button(esc_attr__('Submit', 'homepress'), array('primary', 'large', 'stm-admin-button', 'stm-admin-large-button')); ?>
    </form>
    ...
    <?php if (empty($auth_code)) { ?>
        <h3><?php esc_html_e('Instructions For Generating A Token', 'homepress'); ?></h3>
        <ol>
            <li><?php printf(wp_kses_post(__('Click on this <a href="%s" target="_blank">Generate A Personal Token</a> link. <strong>IMPORTANT:</strong> You must be logged into the same Themeforest account that purchased %s. If you are logged in already, look in the top menu bar to ensure it is the right account. If you are not logged in, you will be directed to login then directed back to the Create A Token Page.', 'homepress')), 'https://build.envato.com/create-token/?purchase:download=t&purchase:verify=t&purchase:list=t', $theme_name); ?></li>
            <li><?php echo wp_kses_post(__('Enter a name for your token, then check the boxes for <strong>View Your Envato Account Username, Download Your Purchased Items, Verify Purchases You\'ve Made</strong> and <strong>List Purchases You\'ve Made</strong> from the permissions needed section. Check the box to agree to the terms and conditions, then click the <strong>Create Token button</strong>', 'homepress')); ?></li>
            <li><?php echo wp_kses_post(__('A new page will load with a token number in a box. Copy the token number then come back to this registration page and paste it into the field below and click the <strong>Submit</strong> button.', 'homepress')); ?></li>
            <li><?php echo wp_kses_post(__('You will see a green check mark for success, or a failure message if something went wrong. If it failed, please make sure you followed the steps above correctly.', 'homepress')); ?></li>
        </ol>
    <?php } ?>
