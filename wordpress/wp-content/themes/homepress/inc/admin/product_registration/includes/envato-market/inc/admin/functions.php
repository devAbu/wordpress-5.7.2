...
function envato_market_themes_column($group = 'install')
{
    ...
    foreach ($premium as $slug => $theme) :
        ...
        if (true === $theme['hasUpdate']) {
            ...
            if (current_user_can('update_themes')) {
                ...
                $updateActions['update'] = sprintf(
                    ...
                    esc_attr__('Update %s now', 'homepress'),
                    esc_attr($name),
                    esc_attr($slug),
                    esc_attr($theme['version']),
                    esc_html__('Update Available', 'homepress')
                );
                ...
                $updateActions['details'] = sprintf(
                    ...
                    esc_attr($name),
                    sprintf(
                        wp_kses_post(__('View version %1$s details.', 'homepress')),
                        $theme['version']
                    )
                );
            }
        ...
        if ('active' === $group && current_user_can('edit_theme_options') && current_user_can('customize')) {
            ...
            $actions['customize'] = '<a href="' . esc_url($customize_url) . '" class="button button-primary load-customize hide-if-no-customize"><span aria-hidden="true">' . esc_html__('Customize', 'homepress') . '</span><span class="screen-reader-text">' . sprintf(esc_html__('Customize &#8220;%s&#8221;', 'homepress'), $name) . '</span></a>';
            ...
            if ($can_activate && current_user_can('switch_themes')) {
                ...
                // Activate link.
                $actions['activate'] = '<a href="' . esc_url($activate_link) . '" class="button"><span aria-hidden="true">' . esc_html__('Activate', 'homepress') . '</span><span class="screen-reader-text">' . sprintf(esc_html__('Activate &#8220;%s&#8221;', 'homepress'), $name) . '</span></a>';
                ...
                if (current_user_can('edit_theme_options') && current_user_can('customize')) {
                    ...
                    $actions['customize_preview'] = '<a href="' . esc_url($preview_url) . '" class="button button-primary load-customize hide-if-no-customize"><span aria-hidden="true">' . esc_html__('Live Preview', 'homepress') . '</span><span class="screen-reader-text">' . sprintf(esc_html__('Live Preview &#8220;%s&#8221;', 'homepress'), $name) . '</span></a>';
                }
            }
			...
			<a href="' . wp_nonce_url($install_link, 'install-theme_' . $theme['id']) . '" class="button button-primary">
				<span aria-hidden="true">' . esc_html__('Install', 'homepress') . '</span>
				<span class="screen-reader-text">' . sprintf(esc_html__('Install %s', 'homepress'), $name) . '</span>
			</a>';
        }
        ...
        /*Changelog*/
        $actions['changelog'] = '
		    <a href="https://stylemixthemes.com/changelogs/homepress" target="_blank" class="button button-secondary">
                <span>' . __('Changelog', 'homepress') . '</span>
			</a>';
            ...
            <div class="<?php echo esc_attr(implode(' ', $classes)); ?>">
                <div class="envato-card-top">
                    <a href="<?php echo esc_url($url); ?>" class="column-icon">
                        <img src="<?php echo esc_url($theme['thumbnail_url']); ?>" alt="<?php echo esc_attr($name); ?>" />
                    </a>
                    <div class="column-name">
                        <h4>
                            <a href="<?php echo esc_url($url); ?>"><?php echo esc_html($name); ?></a>
                            <span class="version"
                                  aria-label="<?php esc_attr_e('Version %s', 'homepress'); ?>"><?php echo esc_html(sprintf(esc_html__('Version %s', 'homepress'), $version)); ?></span>
                        </h4>
                    </div>
                    <div class="column-description">
                        ...
                        <p class="author">
                            <cite><?php esc_html_e('By', 'homepress'); ?><?php echo wp_kses_post($author_link); ?></cite>
                        </p>
                    </div>
    ...
    foreach ($premium as $slug => $plugin) :
        ...
        if ('active' === $group) {
			...
			<a href="' . wp_nonce_url($deactivate_link, 'deactivate-plugin_' . $slug) . '" class="button">
				<span aria-hidden="true">' . esc_html__('Deactivate', 'homepress') . '</span>
				<span class="screen-reader-text">' . sprintf(esc_html__('Deactivate %s', 'homepress'), $name) . '</span>
			</a>';
        } elseif ('installed' === $group) {
            if (!is_multisite() && current_user_can('delete_plugins')) {
				...
				<a href="' . wp_nonce_url($delete_link, 'bulk-plugins') . '" class="button-delete">
					<span aria-hidden="true">' . esc_html__('Delete', 'homepress') . '</span>
					<span class="screen-reader-text">' . sprintf(esc_html__('Delete %s', 'homepress'), $name) . '</span>
				</a>';
            }
            ...
            if (!is_multisite() && current_user_can('activate_plugins')) {
				...
				<a href="' . wp_nonce_url($activate_link, 'activate-plugin_' . $slug) . '" class="button">
					<span aria-hidden="true">' . esc_html__('Activate', 'homepress') . '</span>
					<span class="screen-reader-text">' . sprintf(esc_html__('Activate %s', 'homepress'), $name) . '</span>
				</a>';
            }
            ...
            if (is_multisite()) {
                if (current_user_can('manage_sites')) {
					...
					<a href="' . esc_url(admin_url(wp_nonce_url('plugins.php?action=activate&amp;plugin=' . urlencode($slug), 'activate-plugin_' . $slug))) . '" class="button">
						<span aria-hidden="true">' . esc_html__('Site Activate', 'homepress') . '</span>
						<span class="screen-reader-text">' . sprintf(esc_html__('Site Activate %s', 'homepress'), $name) . '</span>
					</a>';
                }
                ...
                if (current_user_can('manage_network_plugins')) {
					...
					<a href="' . esc_url(network_admin_url(wp_nonce_url('plugins.php?action=activate&amp;plugin=' . urlencode($slug), 'activate-plugin_' . $slug))) . '" class="button">
						<span aria-hidden="true">' . esc_html__('Network Activate', 'homepress') . '</span>
						<span class="screen-reader-text">' . sprintf(esc_html__('Network Activate %s', 'homepress'), $name) . '</span>
					</a>';
                }
            }
            // @codeCoverageIgnoreEnd
        ...
        <div class="col" data-id="<?php echo esc_attr($plugin['id']); ?>">
            <div class="<?php echo esc_attr(implode(' ', $classes)); ?>">
                <div class="envato-card-top">
                    <a href="<?php echo esc_url($url); ?>" class="column-icon">
                        <img src="<?php echo esc_url($plugin['thumbnail_url']); ?>" alt="<?php echo esc_attr($name); ?>" />
                    </a>
                    <div class="column-name">
                        <h4>
                            <a href="<?php echo esc_url($url); ?>"><?php echo esc_html($name); ?></a>
                            <span class="version"
                                  aria-label="<?php esc_attr_e('Version %s', 'homepress'); ?>"><?php echo esc_html(sprintf(esc_html__('Version %s', 'homepress'), (isset($plugins[$slug]) ? $plugins[$slug]['Version'] : $version))); ?></span>
                        </h4>
                    </div>
                    <div class="column-description">
                        <div class="description">
