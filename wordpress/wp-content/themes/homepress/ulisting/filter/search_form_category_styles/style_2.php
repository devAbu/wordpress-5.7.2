    ...
    if( $search_fields = $listingsType->getSearchFields( StmListingType::SEARCH_FORM_CATEGORY, null, true) ) {
        foreach ( $search_fields as $field ) {
            ...
            if( $field[ 'field_type' ] == StmListingType::SEARCH_FORM_TYPE_RANGE AND !empty( $field[ 'attribute_name' ] ) ) {
                ...
                $listing_type_data[ 'listing_type_' . $listingsType->ID ][ $field[ 'attribute_name' ] ] = array( $min, $max );
                $listing_type_form[ $listingsType->ID ][] = [
                    "label" => $field[ 'label' ],
                    "type" => $field[ 'type' ],
                    "template" => "" . StmListingTemplate::load_template( 'components/fields/' . $field[ 'type' ],
                    ...
                    <?php foreach ( $categories as $k => $category ) : ?>
                        <div data-v-if="active_tab == <?php echo esc_attr( $category->term_id ); ?>"
                            ...
                            <div class="row">
                                ...
                                <?php else: ?>
                                    <div class="col-xl-3 col-lg-3 col-md-3 col-sm-12 listing-type-box">
                                    ...
                                    <?php foreach ( $listing_type_form as $key => $form ) :
                                        ...
                                        if( $index == 0 ) {
                                            ...
                                            $form_content .= '<div data-v-if="category_selected.type_selected == ' . $key . '" class="col-xl-2 col-lg-2 col-md-3 col-sm-12">
                                                                        <a data-v-bind_href="category_selected.url" class="homepress-button homepress-button-full icon-left" >' . __( "Search", "homepress" ) . '</a>
                                                                  </div>';

                                            $form_content .= '<div data-v-if="category_selected.type_selected == ' . $key . '" class="advanced-search-button" data-v-bind_class="{open:categories[' . $category->term_id . '].advanced_panel_show}"
