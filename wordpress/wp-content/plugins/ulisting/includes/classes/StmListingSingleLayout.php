...
class StmListingSingleLayout {
    ...
    public static function uListing_active_single_template() {
        ...
        $post_id   = isset( $_POST['listing_type_id'] ) ? $_POST['listing_type_id'] : null;
	...
	public static function save_layout(){
		...
		} if ( isset($request_data['listing_type_id']) and $listing_type = StmListingType::find_one($request_data['listing_type_id']) ) {
			update_post_meta(
				$listing_type->ID,
	...
	public static function get_layout_list($type_id = null) {
		...
		if ( ! empty( $type_id ) ) {
		    ...
		    $id = isset( $_GET['listing_type_id'] ) ? $_GET['listing_type_id'] : null;
        }
	...
	public static function get_layout_delete() {
        ...
        if ( current_user_can('manage_options') && isset($_POST['id']) AND $listing_type = StmListingType::find_one($_POST['id'])){
            delete_post_meta($listing_type->ID, $_POST['layout_id']);
            $result['success'] = true;
...
//            StmVerifyNonce::verifyNonce(sanitize_text_field($_POST['nonce']), 'ulisting-ajax-nonce');
            ...
            if ( !empty($files['file']) && !empty($_POST['id']) && file_exists($files['file']['tmp_name']) ) {
                ...
                if ( isset($_POST['listing_type_id']) && isset($_POST['type']) && $_POST['type'] === 'single' ) {
                    update_post_meta(intval($_POST['listing_type_id']), sanitize_text_field($_POST['id']), $content);
                    $layouts = self::get_layout_list($_POST['listing_type_id']);
                    $layouts = isset($layouts['layouts']) ? $layouts['layouts'] : [];

                } elseif ( isset($_POST['type']) && $_POST['type'] === 'inventory' ) {
                    update_option(sanitize_text_field($_POST['id']), $content);
    ...
    public static function uListing_save_single_layout() {
        ...
        $post_id   = isset($request_data['listing_type_id']) ? $request_data['listing_type_id'] : null;
        ...
        $section   = isset($request_data['sections'])  ? $request_data['sections'] : null;
	...
	public static function get_builder_data(){
		...
		if(isset($request_data['listing_type_id']) AND $listingType = \uListing\Classes\StmListingType::find_one($request_data['listing_type_id'])) {
