...
//            add_action('save_post', [self::class, 'action_save_post'], 10, 3);
    ...
    {
        echo "<script type='text/javascript'> var currentAjaxUrl = '" . admin_url('admin-ajax.php', 'relative') . "' </script>";
    }
    ...
    public static function listing_post_type_save() {
        ...
        $data    = $_POST;
        $post_ID = isset($data['id']) ? $data['id'] : null;
        if ( current_user_can('manage_options') && !empty($post_ID) && ($listingType = StmListingType::find_one($post_ID)) ) {
            ...
            if ( isset( $data['similar_listing'] ) )
                update_post_meta($post_ID, 'uListing_similar_listing_data', $data['similar_listing']);
    ...
    public static function action_save_post($post_ID, $post, $update)
    {
        if (isset($_POST['post_type']) AND $_POST['post_type'] == 'listing_type') {
            if(isset($_POST['StmListingTypeListingSimilarAttribute'])){
                ...
                update_post_meta($post_ID, 'ulisting_listing_similar_attribute', $submit_form);
            } else
    ...
    {
        if ($data) {
            update_post_meta($this->ID, 'stm_listing_order', $data);
        } else
    ...
    {
        global $wp_admin_bar;
        if(get_post_type() === "listing_type" || get_post_type() === "stm_pricing_plans" || (isset($_GET['taxonomy']) && ($_GET['taxonomy'] === 'listing-region' || $_GET['taxonomy'] === 'listing-category')))
    ...
    public function saveSearchForms($data, $search_data = [], $post_ID = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (!empty($fields))
                update_post_meta($this->ID, $key, $fields);
        }
    ...
    public static function listing_basic_form()
    {
        ...
        $elem_attr = [];
        $feature_models = [];
        $listingType = StmListingType::find_one($query_params['listing_type']);
    ...
    public static function stm_update_listing_type_attr(){
        ...
        if(isset($data['post_ID'])){
            ...
            if (isset($data['attribute'])) {
                $attributes = (is_array($_POST['attribute'])) ? ulisting_sanitize_array($data['attribute']) : [];
                update_post_meta($post_ID, 'listing_type_attribute', $attributes);
            } else
            ...
            if (isset($data['attribute_required'])) {
                ...
                update_post_meta($post_ID, 'stm_listing_type_attribute_require', $attribute_required);
            } else
    ...
    public static function my_listing_list()
    {
        ...
        $limit = 9;
        $view_type = 'list';
        $user = new StmUser($query_params['user_id']);
        $listing_types = ulisting_all_listing_types();
        $status = isset($query_params['status']) ? $query_params['status'] : '';
        $query_var = isset($query_params['query_var']) ? $query_params['query_var'] : [];
        $pagination_settings = isset($query_params['pagination_settings']) ? $query_params['pagination_settings'] : [];
    ...
    public function availableSortField()
    {
        ...
        foreach ($this->getAttribute() as $attribute) {
            if ($attribute->type == 'gallery' OR $attribute->type == 'wp_editor' OR $attribute->type == 'file')
                continue;
            $fields[$attribute->name] = __($attribute->title, "ulisting");
        }
        return $fields;
    }
