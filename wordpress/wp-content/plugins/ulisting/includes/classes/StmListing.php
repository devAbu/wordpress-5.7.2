...
class StmListing extends StmBaseModel
{
    ...
    public static function ulisting_filter_post_type_by_lisitng_type()
    {
        ...
        if ($typenow == $post_type) {
            ...
            echo "<select name='". $listing_type_post_type ."'>";
            echo "<option value='0'>" . __("Show All Listing Types") . "</option>";
            foreach ($listing_types as $listing_type) {
                $active = ($selected == $listing_type->ID) ? 'selected' : '';
                echo "<option " . $active . " value='" . $listing_type->ID . "'>" . $listing_type->post_title . "</option>";
            }
            echo "</select>";
        };
    }
    ...
    public static function ulisting_filter_post_type_by_post_author()
    {
        ...
        if ($typenow == $post_type) {
            ...
            echo "<select name='". $listing_post_author ."'>";
            echo "<option value='0'>". __("Show All Users") ."</option>";
            foreach ($users as $user){
                ...
                echo "<option value='" . $data->ID . "' " . $active . ">" . $title . "</option>";
            }
            echo "</select>";
        }
    }
    ...
    public static function stm_listing_columns_content($column_name, $post_ID)
    {
        ...
        if ($column_name == 'image') {
            ...
            if ($thumbnail_url) {
                ...
                echo "<img style='width:100px' src='" . ulisting_get_placeholder_image_url() . "'>";
        }
    }
    ...
    public static function uListing_listing_data() {
        ...
        if ( current_user_can('manage_options') && isset( $_GET['nonce'] ) ) {
            ...
            $post_id        = isset( $_GET['listing_id'] ) ? $_GET['listing_id'] : '';        // current listing post id
            $options        = [];                                                            // listing type options
            $feature_value  = false;                                                            // is post featured
            $selected_type  = '';                                                          // listing type id
            $created_by     = '';                                                         // post author
            $user_list      = [];                                                        // wp users list
            $listing_types  = ulisting_all_listing_types();                             // listing types list
            $post_link      = '';
            $thumbnail      = null;
            $listingType    = null;
            ...
            if ( $listing = StmListing::find_one($post_id) ) {
                if ( $listingType   = $listing->getType() ) {
    ...
    public static function uListing_listing_link_save() {
        ...
        if ( current_user_can('manage_options') && isset( $request_data['nonce'] ) ) {
            ...
            $post_id = isset($request_data['post_id']) ? $request_data['post_id'] : null;
            $listing = StmListing::find_one($post_id);

            if ( $listing && !empty($request_data['base_url']) ) {
                $listing->guid = esc_url($request_data['base_url']);
    ...
    public static function uListing_listing_save() {
        ...
        if ( current_user_can('manage_options') && isset( $request_data['nonce'] ) ) {
            ...
            $post_id = isset($request_data['post_id']) ? $request_data['post_id'] : null;
            $listing = StmListing::find_one($post_id);
            $title   = isset($request_data['title']) ? $request_data['title'] : __('Auto Draft', 'ulisting');


            if ( isset($request_data['is_create']) ) {
                $post_name          = str_replace(' ', '-', strtolower($title));
                $listing->post_name = $post_name;
                $listing->guid      =  get_site_url() . '/listing/' . $post_name;
            }
            ...
            if ( $listing ) {
                ...
                if ( !empty($request_data['post_status']) && in_array($request_data['post_status'], [self::STATUS_PUBLISH, self::STATUS_PENDING, self::STATUS_DRAFT]) )
                    $listing->post_status = $request_data['post_status'];
                ...
                $options = isset($request_data['options']) ? $request_data['options'] : [];
                if ( !empty($options) ) {
                ...
                if ( !empty($request_data['base_url']) && !isset($request_data['is_create']) )
                    $listing->guid = esc_url($request_data['base_url']);
                ...
                if ( isset($request_data['listing_meta']) )
                    foreach (ulisting_sanitize_array($request_data['listing_meta']) as $key => $meta)
                        update_post_meta($listing->ID, $key, $meta);
    ...
    public static function uListing_get_selected_type_options() {
        ...
        if ( current_user_can('manage_options') && isset( $_GET['nonce'] ) ) {
            ...
            $listing = isset( $_GET['listing_id'] ) ? $_GET['listing_id'] : '';
    ...
    public static function action_save_post($post_ID, $post, $update)
    {
        ...
        if (isset($_POST['post_type']) AND $_POST['post_type'] == 'listing') {
    ...
    public static function update_ids($id, $data) {
        ...
        update_option('gallery_store', $ids);
    }
    ...
    public static function listing_file_ajax() {
        ...
        $options = ulisting__sanitize_array(isset($_POST['options']) ? $_POST['options'] : []);
    ...
    public static function getAttributesCriteria($params = null)
    {
        ...
        foreach ($params as $key => $value) {
            ...
            switch ($key) {
                case 'date_range':
                    $date = current($value);
                    $criteria['where'] .= " \n {$and} ( list_attr_rel_" . $i . ".attribute = '" . key($value) . "' AND  
									   (
								          list_attr_rel_" . $i . ".value >= '{$date[0]}' AND
								          list_attr_rel_" . $i . ".value <= '{$date[1]}'
									   ))";
                    break;
                case 'range':
                    foreach ($value as $attr_key => $attr) {
                        ...
                        $criteria['where'] .= " \n {$and} ( list_attr_rel_" . $attr_key . ".attribute = '" . $attr_key . "' AND
									   (
								          list_attr_rel_" . $attr_key . ".value >= {$date[0]} AND
								          list_attr_rel_" . $attr_key . ".value <= {$date[1]}
									   ))";
                        $and = " AND ";
                    }
                    break;
                default:
                    if (is_array($value)) {
                        $value = implode(',', $value);
                        $where = " \n {$and} ( list_attr_rel_" . $i . ".attribute = '{$key}' AND  list_attr_rel_" . $i . ".value in ({$value}))";
                    ...
                    if($value !== false)
                    {
                        $criteria['where'] .= " \n {$and} ( list_attr_rel_" . $i . ".attribute = '{$key}' AND  list_attr_rel_" . $i . ".value = '{$value}' )";
                    }
            }
        }
        return $criteria;
