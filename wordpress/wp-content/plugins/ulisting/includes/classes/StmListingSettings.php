...
//        $result[''] = __('Select Page', 'ulisting');
    ...
    public static function stm_plugin_settings() {
        ...
        if ( current_user_can('manage_options') && (isset($_GET['action']) && $_GET['action'] === 'stm_plugin_settings') ) {
    ...
    public static function stm_settings_save() {
        ...
        if ( current_user_can('manage_options') && (isset($_POST['action']) && $_POST['action'] === 'stm_settings_save') ) {
    ...
    public static function stm_save_template() {
        ...
        if ( current_user_can('manage_options') && (isset($_POST['action']) && $_POST['action'] === 'stm_save_template') ) {
            ...
            if ( !empty( $_POST['slug'] ) && !empty( $_POST['data'] ) && isset( $email_store[$_POST['slug']] ) ) {
                $result['status']  = 'success';
                $result['message'] = self::plugin_text_domain('Template updated successfully');
                $email_store[$_POST['slug']] = $_POST['data'];
    ...
    public static function stm_save_payment() {
        ...
        if ( current_user_can('manage_options') && !empty($_POST['id']) ) {
            ...
            if ( $id === 'stripe' )
                Stripe::save_settings( $_POST['data'] );

            if ( $id === 'paypal_standard' )
                PayPalStandard::save_settings( $_POST['data'] );

            if ( $id === 'paypal' ) {
                PayPal::save_settings($_POST['data']);
            }
    ...
    public static function stm_generate_pages() {
        ...
        if ( current_user_can( 'manage_options' ) && isset($_POST['pages']) && isset($_POST['action']) && $_POST['action'] === 'stm_generate_pages' ) {
    ...
    public static function install_default_currency()
    {
        if (!get_option("stm_currency_page")) {
            ...
            update_option(self::ULISTINGCURRENCY_SETTINGS, $default_settings);
        }
    }
